...
/* If defined, will use gettimeofday, rather than clock_gettime */
// #define USE_TOD
...
double core_mhz(int verbose) {
	...
	if (strstr(buf, "cpu MHz")) {
	    if (sscanf(buf, "cpu MHz\t: %lf", &cpu_mhz) == 1)
		break;
	}
    }
    fclose(fp);
    if (cpu_mhz == 0.0) {
	fprintf(stderr, "Can't find clock information in /proc/cpuinfo\n");
	cpu_mhz = 1000.0;
	return cpu_mhz;
    }
    if (verbose) {
